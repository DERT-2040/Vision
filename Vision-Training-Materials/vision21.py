...
def show_center_data(listCenterX,listCenterY,imgOriginal,status,vp):
...
def show_dist_data(imgOriginal, listCenterX,vp): # pixels to inches
	if len(listCenterX)>= 2:	
		...
		vp.putNumber("Dist to Target = ",calc_tgt_dist)
	else :
...
def calc_percent_error_X(resolutionX, listCenterX):
	center = 320
