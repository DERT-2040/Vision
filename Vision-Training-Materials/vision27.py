...
def show_center_data(listCenterX,listCenterY,imgOriginal,tgtStatus,vp):
	if len(listCenterX)>= 2:	
		cv2.putText(imgOriginal, tgtStatus, (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 255), 2)
		...
		vp.putNumber("Ctr X = ", cX)
		vp.putNumber("Ctr Y = ", cY)
		...
		vp.putNumber("Ctr X = ", 0)
		vp.putNumber("Ctr Y = ", 0)
...
def show_dist_data(imgOriginal, listCenterX,vp): # pixels to inches
	if len(listCenterX)>= 2:	
		...
		tgtDist = 5123.3 * (math.pow((horiLine),(-1)))
		tgtDist = round(tgtDist,2)
