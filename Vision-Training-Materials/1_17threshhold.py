...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	imgThresh = cv2.inRange(imgHSV, np.array([35,100,100]), np.array([90, 255, 255]))
	...
	for c in cnts:
		...
		if len(approx) >= 4 and len(approx) <= 10:
			...
			keepSolidity = solidity > .1 and solidity < 4
			keepAspectRatio = aspectRatio >= 1 and aspectRatio <=25
