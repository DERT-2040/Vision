...
def show_angle_data(listArea, listCenterX,imgOriginal,vp):
	...
	position = 2
	if len(listArea) > 1:
		diffSizeCont = abs(((listArea[1]-listArea[0])/listArea[0])*100)
		...
		if tgtAng > 50:
			...
			position = -1
		elif tgtAng > -50 and tgtAng < 50:
			position = 0
		cv2.putText(imgOriginal, ("Position = " + str(position)), (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 0), 1)
		vp.putNumber("position", position)
	else:
		cv2.putText(imgOriginal, ("Position = "), (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 0), 1)
		vp.putNumber("position", position)
	return imgOriginal
