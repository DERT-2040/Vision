	...
	for c in cnts:
		...
		if len(approx) >= target.vertices_lower and len(approx) <= target.vertices_upper:
			...
			if keepDims and keepSolidity and keepAspectRatio:
				...
				ax,ay,aw,ah,imgOriginal = vision.bounding_rectangle_blue(imgOriginal,c)
				listCenterX = []
				listCenterY = []
				M = cv2.moments(approx)
				listCenterX, listCenterY, imgOriginal = vision.center_of_contour(imgOriginal,approx,w,h)
	# draw the status text on the frame	
	#imgOriginal = vision.draw_box(imgOriginal)
	#imgOriginal = vision.show_center_data(listCenterX,listCenterY,imgOriginal)
	#imgOriginal = vision.show_dist_data(imgOriginal,listCenterX)
